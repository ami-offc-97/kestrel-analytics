...
**Kestrel Provisions — the investigation behind the numbers**

This is the working record of what we found in the four source feeds, feed by
feed. It is the evidence base for [`../DECISIONS.md`](../DECISIONS.md) (which
summarises) and for the limitations recorded against each metric in
[`kpi_catalogue.md`](kpi_catalogue.md).

Everything here was verified two ways before being written down: by reading how
the column is produced in `generate_dataset.py`, and by measuring the
consequence against the built tables. Where a finding contradicts
`docs/assignment/02_Feed_Contracts.md`, the data wins — that document warns it
is partial and partly wrong, and several statements in it are.

Where an earlier conclusion here was itself wrong, the correction is left
visible rather than quietly edited out. Two are marked as superseding earlier
placeholders.

**Runnable proofs** for the three findings that block a requested metric live
in [`../sql/queries/limitations/`](../sql/queries/limitations/).
