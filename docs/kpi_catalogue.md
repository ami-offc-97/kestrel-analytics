...
[Section 7: Requested but not buildable](#7-requested-but-not-buildable), with
...
| | |
|---|---|
| **Definition** | Whether each source feed delivered what it was supposed to, per day: nothing short against the ingestion job's own manifest, no absent days, and no holes inside a feed that looks healthy in total. |
| **Grain** | Three grains, because no single one is sufficient: (feed, partition) against the manifest; (feed, calendar_date) against a calendar spine; (gateway_id, calendar_date) for within-feed holes. |
| **Formula** | See `dq_partition_reconciliation`, `dq_feed_completeness`, `dq_telemetry_gateway_health`. Status and severity are derived columns on each. |
| **Filters / exclusions** | The known-truncated Parquet file is excluded *by name* when counting rows, exactly as staging does — otherwise `read_parquet` fails and no check runs at all. Its shortfall is still reported in full; skipping the file suppresses the crash, not the finding. |
| **Source feeds** | All six raw feeds, plus `data/_manifest/expected_partitions.csv` |
| **Owner** | Data Platform |
| **Query** | `sql/queries/data_quality/feed_completeness.sql` |
| **Known limitation** | **Current state of the shipped dataset: 1 short partition (−945 rows), 3 feeds with no reconciliation baseline, 2 gateway outage days, 0 absent days.** Three limitations worth knowing. (a) **`erp_cdc` cannot be reconciled at all** — the manifest covers only POS, telemetry and WMS, so all three ERP CDC feeds have no baseline even in principle. Reported explicitly as `NO_MANIFEST_BASELINE` rather than left as a silent absence. (b) **Low-volume feeds are not volume-monitored.** `product_master` runs a median of 4 rows/day, where a percentage-of-median test is meaningless; feeds below 100 rows/day are marked `NOT_MONITORABLE_LOW_VOLUME` and only true zero-row days are surfaced, as INFO. Its four zero-row days most likely mean *no product changed*, which is normal CDC behaviour — separating that from a failed extract needs a delivery receipt from the ERP, which we do not have. (c) **Within-feed monitoring exists for telemetry gateways only.** The same spine-times-dimension pattern is needed per till/outlet on POS and per warehouse on WMS before either could be called monitored. |

**Why this metric has three grains, which is the whole design.** Each check catches a class of failure the others structurally cannot see, and the shipped dataset happens to contain one example of each:

- The **manifest** check is the only one that can prove a partition is *short* rather than merely quiet. It isolates the truncated file at 85.9% of expected. No amount of querying the loaded tables would reveal it, because the rows are simply not there to count.
- The **calendar spine** is the only one that can see a day that was never written. The manifest is generated by scanning what *was* written, so an absent day is missing from both sides and reconciles clean.
- The **per-gateway baseline** is the only one that can see a hole inside a healthy-looking feed. The GW-017 outage sits at ~95% of feed median — comfortably normal — because one dead gateway is 2.5% of telemetry volume. Feed-level monitoring would never have found it.

Nor can feed-level thresholds be tuned to compensate: the truncated-file day lands at 85.0% of median while `sales_order_header`'s quietest ordinary day is 85.9%, so no threshold separates the defect from normal variation without firing on both.

**On threshold discipline.** Both detectors were retuned after their first version produced only noise — 74 false positives at gateway grain from an 80% floor that sat inside normal variance (measured: the 0.1st percentile of non-zero gateway-days is 77.2%), and 342 at feed level, all from the two tiny CDC feeds. Both now run at zero false positives across 21,294 gateway-days and 3,276 feed-days. An alert stream that is mostly noise is worse than no monitoring, because it trains people to ignore the one alert that matters.

---

## 7. Requested but not buildable
